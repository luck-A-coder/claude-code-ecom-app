  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/80">
      <div className="hidden items-center justify-center gap-2 bg-foreground py-2 text-xs text-background sm:flex">
        ...
        <span>Free shipping on orders over ฿1,500 · Delivered in 1–2 business days</span>
      </div>
      ...
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:px-6 lg:px-8">
        <Sheet>
          ...
          <SheetContent side="left">
            ...
            <div className="mt-2 flex flex-col gap-1 border-t border-border px-6 pt-4">
              <span className="px-3 text-xs font-medium text-muted-foreground">Categories</span>
              {categories.map((c) => (
                <SheetClose
                  key={c.slug}
                  render={
        ...
        <div className="ml-auto flex items-center gap-2">
          <InputGroup className="hidden w-56 sm:flex md:w-72">
            ...
            <InputGroupInput placeholder="Search products, brands..." />
          </InputGroup>
